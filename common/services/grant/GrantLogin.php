...
class GrantLogin extends GrantBase{
    private $scene = "登录";
    public $sn = '';  // 订单ID
    ...
    public function sendGold($uid, $value){
        MemberAccount::addRowList($uid, $value, MemberAccount::ACCOUNT_POINT, MemberRecord::TYPE_POINT, $this->scene);
    }
    ...
    public function sendCoupon($uid, $cou_id){
        ...
        if( empty($coupon) || $coupon->get_method != Coupon::GET_METHOD_LOGIN){
    ...
    public function sendCash($uid, $value){
        MemberAccount::addRowList($uid, $value, MemberAccount::ACCOUNT_GIVE, MemberRecord::TYPE_CASH, $this->scene);
    }
}
